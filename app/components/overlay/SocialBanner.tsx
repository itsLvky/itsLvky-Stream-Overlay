...
function PlatformIcon({ name, size = 26 }: { name: string; size?: number }) {
  const p = { width: size, height: size, viewBox: '0 0 24 24', fill: '#fff' as const }
  switch (name) {
    case 'twitch':
      return (
        <svg {...p}>
...
function BannerIcon({ icon }: { icon: string }) {
  ...
  if (KNOWN_PLATFORMS.includes(key)) return <PlatformIcon name={key} size={28} />
