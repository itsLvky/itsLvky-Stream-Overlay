...
function Module({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`flex h-8 items-center gap-1.5 rounded-full px-3 font-medium ${className}`}
      style={{
...
// Compact two-row variant of TopBar for narrow (portrait) canvases —
// TopBar's absolutely-centered clock module overlaps the side groups below ~1400px width.
export default function PortraitTopBar() {
  const { connected, viewerCount, streamStartedAt, gameName } = useStreamerbot()
  const now = useClock()
  ...
  const timeStr = now
    ...
    : '--:--:--'
  const dateStr = now ? `${WEEKDAYS[now.getDay()]} ${now.getDate()}. ${MONTHS[now.getMonth()]}` : ''
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Module className="min-w-0">
          <span className="truncate text-sm" style={{ color: '#cccccc' }}>
            {gameName ?? 'Just Chatting'}
          </span>
        </Module>

        <Module className="ml-auto">
          <svg
            width="13"
            height="13"
            viewBox="0 0 24 24"
